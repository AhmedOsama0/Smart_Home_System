...
void main(void)
{
	...
	while(1)
	{
		...
		if(Check_Password(arr0,Local_u8Password) == 1)		//Check if password true
		{
			Local_u8Flag = 1;
		...
		else if(Check_Password(arr1,Local_u8Password) == 1)	//Check if password true
		{
			Local_u8Flag = 1;
		...
		else if(Check_Password(arr2,Local_u8Password) == 1)	//Check if password true
		{
			Local_u8Flag = 1;
		...
		else	//if password false
		{
			...
			{
				CLCD_voidSendCommand(1); 	//Clear LCD
		...
		if(Local_u8Flag == 1)	//If password true
		{
			while(1)
			{
				CLCD_voidSendCommand(1);	//Clear LCD
				...
				Local_u8PressedKey = UART_u8RecieveData(); 	//Get Input from user
				...
				case '1':
					...
					case '1':
						...
						switch(Local_u8PressedKey)
						{
						/*Case 1 for room 1 ON*/
						case '1':
							Room1_ON();
							break;
							/*Case 2 for room 1 OFF*/
						case '2':
							Room1_OFF();
							break;
						}break;
						/*Case 2 for room 2 select*/
						case '2':
							CLCD_voidSendCommand(1);	//Clear LCD
							CLCD_voidPosition(0,0);
							...
							switch(Local_u8PressedKey)
							{
							/*Case 1 for room 2 ON*/
							case '1':
								Room2_ON();

								break;
								/*Case 2 for room 2 ON*/
							case '2':
								Room2_OFF();
								break;
							}break;
							/*Case 3 for Exit light mode*/
							case '3':
								break;
							default:
								CLCD_voidSendCommand(1);	//Clear LCD
					...
					case '2':
						...
						switch(Local_u8PressedKey)
						{
						/*Case 1 for open window*/
						case '1':
							Motor_Clockwise();
							break;
							/*Case 2 for close window*/
						case '2':
							Motor_Anticlockwise();
							break;
						default:
						...
						case'+':
							...
							case '4':
								...
								switch(Local_u8PressedKey)
								{
								/*case 1 for Door open*/
								case '1':
									Servo_SetAngle(180);
									break;
									/*case 2 for Door close*/
								case '2':
									Servo_SetAngle(90);
									break;
								}break;
								/*case 5 for music mode*/
								case '5':
